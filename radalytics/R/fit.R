  ...
  function() {
    ...
    i<-0;while(i<=length(names)){
      if (toString(names[i])!='NA' && toString(names[i])!='') {
        ...
        interpretedMeanValue<-round(mean(groupedList$calc_conc),3)    
        interpretedSd<-round(sd(groupedList$calc_conc),3)       
        interpretedSd[is.na(interpretedSd)]<-NA
        interpretedCv<-round((interpretedSd/interpretedMeanValue)*100,3)        
...
"doublefiveplfit"<-function() {
  ...
  vlim<-as.numeric(vlim)
  ul<-as.numeric(ul)
  superdata<-applyStandards()
  curves<-list()
  resultsets<-list()
  t<-1;while(t<=2){
    secondary_wavelength<-NA
    ...
    #split data array into samples and standards, (controls are treated as samples)
    data<-superdata[superdata[,'sequenceNum']==t,]
    samples<-subset(data,(data$objectType =='control' | data$objectType =='sample') & data$wavelength == primary_wavelength)
    # samples<-rbind(split(data,data$objectType)$sample,split(data,data$objectType)$control)
    standards<-subset(data,data$objectType =='standard' & data$wavelength == primary_wavelength & data$measuredValue)
    #creates a table containing sample type, name and the grouped well values for each specimen
    names<-unique(standards$objectName)
    groupedStandards<-data.frame()
    ...
    i<-0;while(i<=length(names)){
      if (toString(names[i])!='NA' && toString(names[i])!='') {
        objectName<-toString(names[i])
        groupedList<-subset(standards,standards$objectName==toString(names[i]) & standards$measuredValue >0 & standards$wavelength == primary_wavelength)
        measuredMeanValue<-mean(groupedList$measuredValue)    
        ascribedValue<-groupedList[1,]$ascribedValue
        measuredSd<-sd(groupedList$measuredValue)        
        measuredSd[is.na(measuredSd)]<-NA
    ...
    i<-0;while(i<=length(names)){
      if (toString(names[i])!='NA' && toString(names[i])!='') {
        ...
        groupedList<-subset(combined,combined$objectName==toString(names[i]))
        interpretedMeanValue<-round(mean(groupedList$calc_conc),3)  
        measuredMeanValue<-round(mean(groupedList$measuredValue),3) 
        interpretedSd<-round(sd(groupedList$calc_conc) ,3)       
        interpretedSd[is.na(interpretedSd)]<-NA
        interpretedCv<-round((interpretedSd/interpretedMeanValue)*100,3)        
        interpretedCv[is.na(interpretedCv)]<-NA
        #calculates the grouped well sd and adds a column 
        #print(objectName)
        group<-rbind(group,data.frame(objectName,interpretedMeanValue,interpretedSd,interpretedCv,measuredMeanValue))
      }
      i<-i+1
    }
    group$high<-(group$measuredMeanValue>group[group$objectName=='STD5',]$measuredMeanValue)
    results<-merge(combined,group,by.x=5,by.y=1)
    ...
    resultsets<-rbind(resultsets,results)
    t<-t+1
  }
  return(list(curve=curve,results=postprocess(resultsets),curves=curves,resultsets=resultsets))
  
}

#########################################################END OF double 5-PL FUNCTION######################################################################
#########################################################START OF postprocess FUNCTION######################################################################

"postprocess"<-
  function(resultsets) {
    processed<-list()
    names<-unique(resultsets$objectName)
    i<-1;while(i<=length(names)){
      name<-names[i]
      results<-subset(resultsets,resultsets$objectName==name & resultsets$sequenceNum==1 & resultsets$high)
      if(nrow(results) == 0){
        results<-subset(resultsets,resultsets$objectName==name & resultsets$sequenceNum==2)
      }
      processed<-rbind(processed,results)
      i<-i+1
    }
    return(processed)
  }
